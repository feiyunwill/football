"""Public save API: bounded immutable records, progress and optional persistence."""
# 2026-09-10: preserve the old unbounded mutable implementation for review.
# """
...
#             raise ValueError(f"Save slot already exists: {slot_id}")
...
#             return False
#
#         # 2026-09-09: nested saves must not alias mutable caller data.
#         # slot.data = data.copy()
#         encoded = json.dumps(data).encode()
#         slot.data = copy.deepcopy(data)
#         slot.updated_at = time.time()
#         # slot.size_bytes = len(json.dumps(data).encode())
#         slot.size_bytes = len(encoded)
#         if name:
#             slot.name = name
#
#         return True
...
#             )
#             if not isinstance(slot.slot_id, str) or not isinstance(slot.data, dict):
#                 return None
#             if slot.slot_id not in self._slots and len(self._slots) >= self._max_slots:
#                 return None
#             slot.size_bytes = len(json.dumps(slot.data).encode())
#             self._slots[slot.slot_id] = slot
#             return slot
#         # 2026-09-09: malformed types and unknown save kinds are invalid imports.
...
#         # 2026-09-09: snapshots must not mutate live progress through nested aliases.
...
#         # self._progress = data
#         self._progress = copy.deepcopy(data)

from gfootball.frame_sync.save_data import (
    SaveSlot, SaveType, SaveLimits, SaveCapacityError, SaveFormatError, SaveConflictError,
)
from gfootball.frame_sync.save_runtime import SaveManager, GameProgress
from gfootball.frame_sync.save_store import SaveDiskLimits

__all__ = ['SaveSlot', 'SaveType', 'SaveLimits', 'SaveDiskLimits', 'SaveCapacityError',
           'SaveFormatError', 'SaveConflictError', 'SaveManager', 'GameProgress']
