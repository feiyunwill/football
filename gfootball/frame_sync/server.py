...
class BotTakeoverManager(object):
  ...
  # 2026-09-13: preserve simulated three/four-second cooldowns at each authority rate.
  # def __init__(self):
  #   self._bots = {}  # slot_index -> {'team': 0/1, 'shoot_cooldown': int}
  def __init__(self, *, rate_hz=10):
    if type(rate_hz) is not int or not 1 <= rate_hz <= 240:
      raise ValueError('Invalid bot input rate')
    ...
    if attacking:
      if ball_dist > 3.0:
        ...
        if ball_dist < 8.0 and goal_dist < 25.0 and bot['shoot_cooldown'] <= 0:
          buttons |= (1 << 3)  # Shot
          # 2026-09-13: shoot cooldown is three simulated seconds.
          # bot['shoot_cooldown'] = 30
          bot['shoot_cooldown'] = 3 * self._rate_hz
    else:
      ...
      if own_goal_dist < 15.0 and ball_dist < 5.0 and bot['shoot_cooldown'] <= 0:
        ...
        # 2026-09-13: clearance cooldown is four simulated seconds.
        # bot['shoot_cooldown'] = 40
        bot['shoot_cooldown'] = 4 * self._rate_hz
...
# 2026-09-10: use the shared bounded TCP runtime, complete wire parsing,
# exact input ownership and asynchronous cancellation before closing descriptors.
