  ...
  for (unsigned int t1 = 0; t1 < templates.size(); t1++) {
    ...
    for (unsigned int t2 = 0; t2 < templates.size(); t2++) {
      ...
      for (unsigned int direction = 0; direction < 9; direction++) {
        GameLoadCheckpoint("animations.generate");
        DO_VALIDATION;
...
// 2026-09-14: generated animation remains owned until it enters the temporary collection.
//           Animation *gen = new Animation(*templates.at(t1));
          auto generated = std::make_unique<Animation>(*templates.at(t1));
          Animation *gen = generated.get();
...
// 2026-09-14: transfer generated animation only after vector insertion succeeds.
...
void AnimCollection::Load() {
  GameLoadCheckpoint("animations.begin");
  GameLoadCleanup rollback([this] { Clear(); });
  DO_VALIDATION;
  ...
  GameLoadCleanup release_player([&] { if (playerNode) playerNode->Exit(); });
...
*/
  ...
  GameLoadCleanup release_templates([&] { for (auto* animation : templates) delete animation; });
  for (unsigned int i = 0; i < files.size(); i++) {
    DO_VALIDATION;
// 2026-09-14: template files keep ownership if loading throws.
//     Animation *animTemplate = new Animation();
//     animTemplate->Load(files[i]);
//     templates.push_back(animTemplate);
    GameLoadCheckpoint("animations.template");
    auto animTemplate = std::make_unique<Animation>();
    animTemplate->Load(files[i]);
    templates.push_back(animTemplate.get());
    animTemplate.release();
  }

  std::vector<Animation*> autoAnims;
  GameLoadCleanup release_generated([&] { for (auto* animation : autoAnims) delete animation; });
  ...
  for (unsigned int i = 0; i < autoAnims.size(); i++) {
    DO_VALIDATION;
// 2026-09-14: prepared animations transfer to the persistent collection exactly once.
...
//     animation->AddExtension("football", extension);
//     _PrepareAnim(animation, playerNode, bodyParts, nodeMap, false);
    GameLoadCheckpoint("animations.prepare-generated");
    auto mirrored = std::make_unique<Animation>(*autoAnims[i]);
    Animation* animation = mirrored.get();
    std::shared_ptr<FootballAnimationExtension> extension(new FootballAnimationExtension(animation));
    animation->AddExtension("football", extension);
    animation->Mirror();
    _PrepareAnim(animation, playerNode, bodyParts, nodeMap, false);
  ...
  for (unsigned int i = 0; i < files.size(); i++) {
    ...
    if ((omitLuxuryAnims && files[i].find("luxury") != std::string::npos) ||
      ...
      for (int mirror = 0; mirror < 2; mirror++) {
        DO_VALIDATION;
// 2026-09-14: a file animation is owned before parsing, extension setup and preparation.
...
// 
//         _PrepareAnim(animation, playerNode, bodyParts, nodeMap, false);
        GameLoadCheckpoint("animations.file");
        auto pending = std::make_unique<Animation>();
        Animation* animation = pending.get();
        std::shared_ptr<FootballAnimationExtension> extension(new FootballAnimationExtension(animation));
        animation->AddExtension("football", extension);
        animation->Load(files[i]);
        if (mirror == 1) animation->Mirror();
...
// 2026-09-14: the utility model cleanup guard handles both success and cancellation.
//   playerNode->Exit();
// }
  rollback.release();
}
...
void AnimCollection::CrudeSelection(DataSet &dataSet,
  ...
  for (int i = 0; i < animSize; i++) {
    ...
    if (query.byPickupBall == true) {
      ...
      if ((animations[i]->GetVariableRef("outgoing_retain_state") == "" &&
           query.pickupBall == true) ||
          // 2026-09-09: read animation metadata without temporary key/value allocation.
          // (animations[i]->GetVariable("outgoing_retain_state") != "" &&
          (animations[i]->GetVariableRef("outgoing_retain_state") != "" &&
    ...
    if (query.byIncomingBallDirection == true) {
      ...
      Vector3 animBallDirection = GetVectorFromString(animations[i]->GetVariableRef("incomingballdirection"));
      ...
      if (animBallDirection.GetLength() != 0.0f &&
        ...
        radian maxDeviation = fabs(atof(animations[i]->GetVariableRef("incomingballdirection_maxdeviation").c_str()) * pi);
    ...
    if (query.byOutgoingBallDirection == true) {
      ...
      Vector3 animBallDirection = GetVectorFromString(animations[i]->GetVariableRef("balldirection"));
      ...
      radian maxDeviation = fabs(atof(animations[i]->GetVariableRef("outgoingballdirection_maxdeviation").c_str()) * pi);
      if (maxDeviation == 0.0) {
        DO_VALIDATION;
    ...
    if (query.byTripType == true) {
      ...
      if (int(round(atof(animations[i]->GetVariableRef("triptype").c_str()))) != query.tripType) continue;
    }
    ...
    if (query.heedForcedFoot == true) {
      ...
      std::string forcedFoot = animations[i]->GetVariableRef("forcedfoot");
      int which = 0;
      ...
      if (which != 0) {
        ...
        std::string touchFoot = animations[i]->GetVariableRef("touchfoot");
...
void AnimCollection::ProcessState(EnvState *state) {
  ...
  // state->process(size);
  state->processCount(size, animations.size(), animations.size());
  ...
  // 2026-09-09: bound collection size before allocation/reference use.
  // state->process(size);
  state->processCount(size, quadrants.size(), quadrants.size());
