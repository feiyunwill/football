...
// 2026-09-09: bounded shared transport; engine work and snapshots stay at frame boundaries.
...
//
//       // Step the game
//       // 2026-09-09: StepWithInput already advanced this frame.
//       // env_.step();
//
...
//     auto scenario = frame_sync::MakeDefaultScenario(config_.left_agents, config_.right_agents, config_.seed);
...
//     {
//       std::lock_guard<std::mutex> lock(mu_);
//       // 2026-09-09: broadcast precisely the frozen input used by the engine.
//       // inputs = current_inputs_;
//       inputs = frame_inputs_;
//       prev_inputs = previous_inputs_;
//     }
//
//     // 2026-09-09: clients lack delta decoding; send full authority until support is negotiated.
...
//         frame_id_, inputs.data(), static_cast<uint16_t>(inputs.size()), buf.data(), buf.size());
//     // 保存当前帧作为下一帧的上一帧
//     {
//       std::lock_guard<std::mutex> lock(mu_);
...
//     // 2026-09-09: use the same digest as UDP/Python clients.
//     //     uint64_t hash = 14695981039346656037ULL;
//     //     for (unsigned char c : digest) {
//     //       hash ^= c;
//     //       hash *= 1099511628211ULL;
//     //     }
//     uint64_t hash = frame_sync::ComputeStateHash(digest.data(), digest.size());
//
...
//
//       if (fid == frame_id_ && accepting_inputs_) {
...
//   size_t num_slots_;
//   // 2026-09-09: IO reads this counter concurrently with the frame loop.
//   // frame_sync::frame_id_t frame_id_;
//   std::atomic<frame_sync::frame_id_t> frame_id_;
//   bool accepting_inputs_ = false;  // guarded by mu_
//   std::vector<frame_sync::SlotInput> frame_inputs_;
...
int main(int argc, char* argv[]) {
  try {
    ...
    config.frame_rate_hz = frame_sync::NativeMatchContract::kHz;
    config.port = static_cast<unsigned short>(argc >= 2 ? number(argv[1], 65535) : 12345);
