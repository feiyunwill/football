// 2026-09-09: replace the broken local prediction loop with the shared
// TCPFrameClient API; this engine-free executable is a wire diagnostic.
...
unsigned Integer(const char* text, unsigned minimum, unsigned maximum) {
  unsigned value = 0; const auto end = text + std::char_traits<char>::length(text);
  const auto result = std::from_chars(text, end, value);
  if (result.ec != std::errc{} || result.ptr != end || value < minimum || value > maximum)
    throw std::invalid_argument("Invalid client argument");
  return value;
}
}
int main(int argc, char** argv) {
  try {
    if (argc < 3 || argc > 4) throw std::invalid_argument("Usage: frame_sync_client HOST PORT [EXPECTED_SLOT]");
    const auto port = Integer(argv[2], 1, 65535);
    std::optional<unsigned> expected;
    if (argc == 4) expected = Integer(argv[3], 0, 21);
    frame_sync::TCPClientTransport client(argv[1], static_cast<unsigned short>(port));
    if (!client.Connect()) throw std::runtime_error("TCP handshake failed");
    const auto& slots = client.session().slots;
    if (expected && (slots.size() != 1 || slots.front() != *expected))
      throw std::runtime_error("Assigned slot differs from EXPECTED_SLOT");
    if (!client.Ready()) throw std::runtime_error("TCP Ready failed");
    std::signal(SIGINT, Stop); std::signal(SIGTERM, Stop);
    std::fprintf(stderr, "TCP diagnostic connected: slot=%u seed=%u (no game simulation)\n", slots.front(), client.session().seed);
    frame_sync::frame_id_t next = 0;
    size_t received_hashes = 0;
    std::array<frame_sync::SlotInput, 22> inputs{};
    auto last_send = std::chrono::steady_clock::now() - std::chrono::seconds(1);
    while (!stopped && client.connected()) {
      client.Poll();
      frame_sync::TCPAuthority authority;
      while (client.PopAuthority(authority)) {
        if (authority.frame < next) continue;
        if (authority.frame != next) { client.Fail(frame_sync::TCPClientStatus::InvalidMessage); break; }
        ++next;
        std::fprintf(stderr, "TCP authority frame=%u slots=%zu\n", authority.frame, authority.inputs.size());
      }
      frame_sync::frame_id_t hash_frame; uint64_t hash;
      while (client.PopHash(hash_frame, hash)) ++received_hashes;
      const auto now = std::chrono::steady_clock::now();
      if (now - last_send >= std::chrono::milliseconds(100)) {
        if (!client.SendInput(next, slots.data(), inputs.data(), static_cast<uint16_t>(slots.size()))) break;
        last_send = now;
      }
      std::this_thread::sleep_for(std::chrono::milliseconds(1));
    }
    // 2026-09-09: distinguish received packets from actual engine confirmations.
    // std::fprintf(stderr, "TCP diagnostic confirmed=%u received_hashes=%zu status=%d\n", next, received_hashes, int(client.status()));
    std::fprintf(stderr, "TCP diagnostic frames_received=%u received_hashes=%zu status=%d\n", next, received_hashes, int(client.status()));
    client.Close(); return stopped ? 0 : 1;
