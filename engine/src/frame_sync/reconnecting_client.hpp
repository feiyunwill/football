...
// 2026-09-09: replace duplicate unbounded transport and fake state preservation
// with the actual TCPFrameClient snapshot/reconciliation path and bounded retries.
...
namespace frame_sync {
enum class ReconnectState : std::uint8_t {
  kConnected, kDisconnected, kReconnecting, kFailed, kClosed
};
struct ReconnectOptions {
  int max_attempts = 10;
  std::chrono::milliseconds initial_delay{100}, max_delay{5000};
  void Validate() const {
    if (max_attempts < 1 || max_attempts > 32 || initial_delay < std::chrono::milliseconds(1) ||
        max_delay < initial_delay || max_delay > std::chrono::seconds(30))
      throw std::invalid_argument("Invalid reconnect retry budget");
  }
};

// Single-owner API: Poll/Tick advance automatic recovery. Backoff never sleeps;
// one due attempt uses TCPFrameClient's existing synchronous Connect/Resume.
// DNS/TCP establishment therefore still needs the later asynchronous network
// deadline contract. Callbacks may Close() safely; nested work is ignored.
//
// The host must supply the token assigned to THIS session by its authority.
// Do not derive a credential from public slot/seed values. The legacy wire
// protocol does not issue tokens; authenticated issuance remains network work.
// The old constructor without real engine callbacks could not restore state and
// has intentionally been removed (no repository callers used that API).
class ReconnectingClient {
 public:
  ...
  struct Callbacks {
    std::move_only_function<void()> on_disconnect;
    std::move_only_function<void(int)> on_reconnect_attempt;
    std::move_only_function<void()> on_reconnect_success;
    std::move_only_function<void()> on_reconnect_failed;
  };
  ReconnectingClient(std::string host, unsigned short port, TCPFrameClient::PrepareEngine prepare,
                     session_token_t token, ReconnectOptions options = ReconnectOptions{},
                     Callbacks callbacks = Callbacks{}, TCPClientLimits limits = TCPClientLimits{},
                     SnapshotBudget snapshots = SnapshotBudget{}, Now now = [] { return Clock::now(); })
      : client_(std::move(host), port, std::move(prepare), limits, snapshots), token_(token),
        options_(options), callbacks_(std::move(callbacks)), now_(std::move(now)) {
    options_.Validate();
    if (!token_ || !now_) throw std::invalid_argument("Reconnect needs a session token and clock");
  }
  ...
  bool Connect() {
    ...
    Guard guard(busy_);
    ++generation_; attempts_ = 0; have_session_ = false;
    const auto generation = generation_;
    const bool success = client_.Connect();
    // Engine preparation is host code and may intentionally close the wrapper.
    if (generation != generation_) { client_.Close(); return false; }
    if (success) { have_session_ = true; state_ = ReconnectState::kConnected; return true; }
    last_error_ = client_.status();
    BeginRetry();
    if (state_ == ReconnectState::kFailed) Notify(callbacks_.on_reconnect_failed);
    return false;
  }
  void Poll() {
    if (busy_) return;
    Guard guard(busy_);
    if (state_ == ReconnectState::kConnected) { client_.Poll(); ObserveDisconnect(); }
    else if (state_ == ReconnectState::kReconnecting) RetryDue();
  }
  FrameSimulation::TickResult Tick(const SlotInput& input, int max_predict = MAX_PREDICT_AHEAD_FRAMES) {
    if (busy_) return {};
    Guard guard(busy_);
    if (state_ == ReconnectState::kReconnecting) { RetryDue(); return {}; }
    if (state_ != ReconnectState::kConnected) return {};
    auto result = client_.Tick(input, max_predict);
    ObserveDisconnect();
    return result;
  }
  bool SendInput(const SlotInput& input) {
    if (busy_ || state_ != ReconnectState::kConnected) return false;
    Guard guard(busy_);
    const bool sent = client_.SendInput(input);
    ObserveDisconnect(); return sent;
  }
  // Request recovery explicitly, for example after the host changes network
  // route. Preserves the accepted session and schedules snapshot restoration.
  bool ResetForReconnect() {
    if (busy_ || !have_session_ || state_ != ReconnectState::kConnected) return false;
    Guard guard(busy_);
    client_.Close(); last_error_ = TCPClientStatus::IoError;
    BeginRetry(); Notify(callbacks_.on_disconnect); return state_ == ReconnectState::kReconnecting;
  }
  void Close() {
// 2026-09-09: complete shutdown when a terminal wrapper stops polling.
//     ++generation_; client_.Close(); state_ = ReconnectState::kClosed; attempts_ = 0;
    ++generation_; client_.Close(); client_.Poll(); state_ = ReconnectState::kClosed; attempts_ = 0;
  }
  bool connected() const { return state_ == ReconnectState::kConnected && client_.connected(); }
  ReconnectState state() const { return state_; }
  TCPClientStatus status() const { return client_.status(); }
  TCPClientStatus last_error() const { return last_error_; }
  int attempts() const { return attempts_; }
  Clock::time_point next_attempt() const { return next_attempt_; }
  frame_id_t next_frame() const { return client_.next_frame(); }
  frame_id_t confirmed_count() const { return client_.confirmed_count(); }
  int rollback_count() const { return client_.rollback_count(); }
  const TCPSessionInfo& session() const { return client_.session(); }
  TCPClientStats stats() const { return client_.stats(); }
  const FrameSimulation* simulation() const { return client_.simulation(); }
 private:
  struct Guard { bool& value; explicit Guard(bool& value) : value(value) { value = true; } ~Guard() { value = false; } };
  static bool Retryable(TCPClientStatus status) {
    return status == TCPClientStatus::IoError || status == TCPClientStatus::IdleTimeout ||
           status == TCPClientStatus::ResolveFailed || status == TCPClientStatus::ConnectFailed ||
           status == TCPClientStatus::HandshakeTimeout;
  }
  template<class Callback, class... Args> bool Notify(Callback& callback, Args... args) {
    const auto generation = generation_;
    try { if (callback) callback(args...); }
    catch (...) { Close(); state_ = ReconnectState::kFailed; return false; }
    return generation == generation_;
  }
// 2026-09-09: complete shutdown when a terminal wrapper stops polling.
//   void BeginRetry() {
//     attempts_ = 0; delay_ = options_.initial_delay;
  void BeginRetry() {
    // Closed/failed states do not run normal IO; finish posted socket shutdown
    // now so peers observe EOF without waiting for wrapper destruction.
    client_.Poll();
    attempts_ = 0; delay_ = options_.initial_delay;
    state_ = Retryable(last_error_) ? ReconnectState::kReconnecting : ReconnectState::kFailed;
    if (state_ == ReconnectState::kReconnecting) next_attempt_ = now_() + delay_;
  }
  void ObserveDisconnect() {
    if (state_ != ReconnectState::kConnected || client_.connected()) return;
    last_error_ = client_.status(); BeginRetry();
    if (!Notify(callbacks_.on_disconnect)) return;
    if (state_ == ReconnectState::kFailed) Notify(callbacks_.on_reconnect_failed);
  }
  void RetryDue() {
    if (now_() < next_attempt_) return;
    ++attempts_;
    if (!Notify(callbacks_.on_reconnect_attempt, attempts_)) return;
    const auto generation = generation_;
    const bool success = have_session_ ? client_.Resume(token_) : client_.Connect();
    if (generation != generation_) { client_.Close(); return; }
    if (success) {
      have_session_ = true; state_ = ReconnectState::kConnected;
      Notify(callbacks_.on_reconnect_success); return;
    }
// 2026-09-09: dispatch the failed attempt shutdown before backoff/terminal state.
//    last_error_ = client_.status();
    last_error_ = client_.status(); client_.Poll();
    if (!Retryable(last_error_) || attempts_ >= options_.max_attempts) {
      state_ = ReconnectState::kFailed; Notify(callbacks_.on_reconnect_failed); return;
    }
    // Saturating addition after strict <=30s validation: no shifts or overflow.
    delay_ = std::min(delay_ + delay_, options_.max_delay);
    next_attempt_ = now_() + delay_;
  }
  TCPFrameClient client_;
  const session_token_t token_;
  const ReconnectOptions options_;
  Callbacks callbacks_;
  Now now_;
  ReconnectState state_ = ReconnectState::kDisconnected;
  TCPClientStatus last_error_ = TCPClientStatus::Closed;
  Clock::time_point next_attempt_{};
  std::chrono::milliseconds delay_{0};
  uint64_t generation_ = 0;
  int attempts_ = 0;
  bool have_session_ = false, busy_ = false;
};
}  // namespace frame_sync
#endif
